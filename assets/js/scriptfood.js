const API_KEYS = [
  "b7ac001729cf4bbc83648cc843576dc7",
  "0088aafe930d4033a5c614a83a670522",
  "19db81f8aeaa44ca9368c08c0050ab3d",
  "292fa4f8908f4c5e9f6c335c46c0daa9",
  "18b31ad0e807468fbe30a17793faa16b",
];
...
function searchFood() {
  ...
  let requestURL =
    ...
    API_KEYS[currentApiKeyIndex] +
    "&query=" +
    userContainerEl.value +
    "&instructionsRequired=true&addRecipeInformation=true&addRecipeNutrition=true";
  ...
  fetch(requestURL).then(function (result) {
    result.json().then(function (results) {
      ...
      if (results.code === 402) {
        ...
        for (
          var i = 0;
          i < results.results[currentItem].analyzedInstructions[0].steps.length;
          i++
        ) {
          ...
          for (
            var j = 0;
            j <
            results.results[currentItem].analyzedInstructions[0].steps[i]
              .ingredients.length;
            j++
          ) {
          ...
          instructionListItem.innerText =
            results.results[currentItem].analyzedInstructions[0].steps[i].step;
        ...
        document.getElementById("foodTitle").innerText =
          results.results[currentItem].title;
        document.getElementById("foodPic").innerHTML =
          "<img src='" + results.results[currentItem].image + "'>";
        console.log("text");
      }
    });
  });
